...
do {
  ...
  Aptabase.shared.trackError(error) // severity: error
}
```
...
```swift
Aptabase.shared.trackError(error, fatal: true) // severity: fatal
```

A few important notes about error reporting:

1. Each report includes the error type, message, stack trace, severity (`error` or `fatal`) and how it was captured (`handled` or `crash`).
2. Reports are sent to a dedicated error endpoint and retried when there are transient failures.
