...
private final class MockErrorURLSession: URLSessionProtocol {
...
final class ErrorDispatcherTests: XCTestCase {
    ...
    var session: MockErrorURLSession!
    ...
    func testFlushShouldPostErrorEndpoint() async throws {
        dispatcher.enqueue(newReport(severity: .fatal, kind: .crash))
    ...
    private func newReport(severity: ErrorSeverity = .error, kind: ErrorKind = .handled) -> ErrorReport {
        ErrorReport.build(
            from: TestError(),
            severity: severity,
            kind: kind,
